...
class KNet(TwoStageDetector):
    ...
    def __init__(self,
        ...
        self.register_buffer(
            'class_num_last', torch.zeros((num_thing_classes), device="cuda"))
        
        self.register_buffer(
            'class_num_now', torch.zeros((num_thing_classes), device="cuda"))
    ...
    def forward_train(self,
        ...
        if cur_iter % K_iter == 0:
            self.class_num_last = self.class_num_now
            self.class_num_now = torch.zeros((80), device=img[0].device)

        for i in range(len(gt_labels)):
            res = torch.unique(gt_labels[i],return_counts=True)
            self.class_num_now[res[0]] += res[1]
        # gt_masks and gt_semantic_seg are not padded when forming batch
        gt_masks_tensor = []
        gt_sem_seg = []
        gt_sem_cls = []
        # batch_input_shape shoud be the same across images
        pad_H, pad_W = img_metas[0]['batch_input_shape']  #896 1344
        assign_H = pad_H // self.mask_assign_stride # 224
        assign_W = pad_W // self.mask_assign_stride # 336

        for i, gt_mask in enumerate(gt_masks):
            mask_tensor = gt_mask.to_tensor(torch.float, gt_labels[0].device) # 1*640*960

...
class KernelIterHead(BaseRoIHead):
    ...
    def forward_train(
        ...
        for stage in range(self.num_stages):
            ...
            mask_results = self._mask_forward(
                stage, x, object_feats, mask_preds, img_metas
            )
            ...
            for i in range(num_imgs):
                if stage < self.assign_stages:
                    ...
                    if prev_cls_score[i] is not None:
                        cls_for_assign = prev_cls_score[i][
    ...
    def copyPaste_test(self, img, img_metas, teacher_info, pseudo_masks, classes_thr):
        ...
        # print(f"cp操作的用时为:{cp_end:.2f}s")

        return img, teacher_info, pseudo_masks
    ...
    def merge_stuff_thing(
        ...
        panoptic_seg = total_masks.new_full((H, W), self.num_classes, dtype=torch.long)

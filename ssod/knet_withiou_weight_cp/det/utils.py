...
def copy_paste_test(i, j, img, teacher_info, det_masks):
    """
    ...
    # dst_img0 = imread("/home/xxxy/cx-project/SSL/SoftTeacher/" + dst_img_fname)
    # 对原图像进行还原（逆归一化操作）
    ...
    _, _, dst_h, dst_w = img.shape
    # 为什么teacher_info mask的h, w和img的不一样
    # 是因为img进行了resize，增强后统一了尺寸吗？
    _, h, w = teacher_info["det_masks"][i].masks.shape  # teacher_info mask的h, w

    t1 = time.time() - start
    # 通过label(int)得类名
    class_name = CocoDataset.CLASSES[int(label)]
    # 由于label与数据集中的类别id不对应，再通过类名得数据集中对应的类别id
    class_id = coco.get_cat_ids(cat_names=class_name)

    # 预测类的JSON文件路径
    # ann_filename = /home/xxxy/cx-project/SSL/SoftTeacher/data/CropBank/train2017.1@10_instances/Annotations + "/" + class_name
    # 加载JSON文件
    # with open(annot_path, 'r') as f:
    #     coco_data = json.load(f)
    # 随机抽取JSON文件中同类别crop
    # ann_num = len(coco_data['annotations'])
    # temp = random.randint(0, ann_num)  # 随机起始的位置（增加随机性？）

    # 获取符合条件的src_img: 1.像素值>1000; 2.与dst_mask的重叠度不超过300个像素点（未实现）
    annotations = coco.anns
    # 获取字典的键
    keys = list(annotations.keys())
    len_ann = len(annotations)
    flag, cnt = 0, 0
    temp = random.randint(0, len_ann)  # 随机起始的位置（增加随机性？）
    for ii in range(temp, len_ann):
        key = keys[ii]
        value = annotations[key]
        if (
            value["category_id"] == class_id[0]
            and value["iscrowd"] == 0
            and 2000 < value["area"] < 5000
        ):
            flag = 1
            src_segmentation = value["segmentation"]
            src_img_id = value["image_id"]
            break
    if flag == 0:
        # print("src_img is not found")
    ...
    # 调整scr_img和scr_mask的尺寸，与dst_mask的大小一致
    src_img_rs = cv2.resize(src_img, (dst_w, dst_h), interpolation=cv2.INTER_NEAREST)
    src_mask_rs = cv2.resize(src_mask, (dst_w, dst_h), interpolation=cv2.INTER_NEAREST)
    # 调整要添加的scr_mask的尺寸，与teacherinfo中dst_mask的大小一致
    src_mask_new = cv2.resize(src_mask, (w, h), interpolation=cv2.INTER_NEAREST)
    # 对dst_img中被遮挡的实例更新掩码和标签
    teacher_info["det_masks"][i].masks = np.where(
        src_mask_new, 0, teacher_info["det_masks"][i].masks
    )

    # 将选择粘贴的实例的像素点叠加到图片上
    img_new = (
        dst_img * (1 - src_mask_rs[..., np.newaxis])
        + src_img_rs * src_mask_rs[..., np.newaxis]
    )
    # 存下来看看
    ...
    img_new_tensor = torch.from_numpy(img_new)

    img_new_tensor = torch.unsqueeze(img_new_tensor, 0).permute(0, 3, 1, 2)
    # img_new_tensor = img_new_tensor.permute(0, 3, 1, 2)
    img_new_tensor = torch.tensor(img_new_tensor, dtype=torch.float32)
    img[i] = img_new_tensor.to("cuda:0")
    ...
    teacher_info["det_labels"] = torch.from_numpy(det_label_new).to("cuda:0")
    det_iou_new = np.concatenate([teacher_info["det_ious"][i].cpu().numpy(), [1.0000]])
    teacher_info["det_ious"] = torch.from_numpy(det_iou_new).to("cuda:0")
    det_scores_new = np.concatenate(
        [teacher_info["det_scores"][i].cpu().numpy(), [1.0000]]
    )
    teacher_info["det_scores"] = torch.from_numpy(det_scores_new).to("cuda:0")

    det_mask_new = teacher_info["det_masks"][i].masks
    teacher_info["det_masks"][i].masks = np.concatenate([det_mask_new, [src_mask_new]])

    return img, teacher_info


def copy_paste_test_2(i, j, img, teacher_info, det_masks):
    """
    输入:原图像文件名,伪标签预测类别名称,预测伪mask
    ...
    # 存下来看看
    # mmcv.imwrite(img_new, out_file + dst_file_name + "_test.jpg")
...
def copy_paste_test_3(i, j, img, teacher_info, det_masks, type):
    """
    输入:原图像文件名,伪标签预测类别名称,预测伪mask
    ...
    label = teacher_info["det_labels"][i][j]
    dst_img_tensor = img[i].cpu().detach()
    class_acc = teacher_info["class_acc"].tolist()
    if all(int(item) == 0 for item in class_acc):
        type = 2
    # 看看用img_id读出来是什么样子
    dst_img_fname = teacher_info["img_metas"][i]["filename"]
    dst_file_path = dst_img_fname.split("/")[-1]
    ...
    if type == 1:
        class_min = class_acc.index(min(class_acc))

...
#查看预测的mask的尺度跟img的尺度----------------------------------mask的大小为ori_shape的大小
    ...
    def get_panoptic_ssl(self, cls_scores, mask_preds, test_cfg, img_meta, num_classes):
        # resize mask predictions back
        scores = cls_scores[:self.num_proposals][:, :self.num_thing_classes]
        thing_scores, thing_labels = scores.max(dim=1)
        stuff_scores = cls_scores[
        ...
        panoptic_result = self.merge_stuff_thing_ssl(total_masks, total_labels,
                                                 total_scores,
                                                 test_cfg.merge_stuff_thing)
        """
        #scores_per_img, topk_indices = total_scores.flatten(
        #    0, 1).topk(
        #        self.test_cfg.max_per_img, sorted=True) 
        scores_per_img, topk_indices = total_scores.topk(
                self.test_cfg.max_per_img, sorted=True) 
        ...
        total_masks = total_masks[mask_indices]  
        _, h, w = total_masks.shape
        bmask = total_masks.cpu().numpy()
        bitmask = BitmapMasks(bmask, h, w)
        
        
        return bitmask, labels_per_img, scores_per_img    

...
class MaskSamplingResult(SamplingResult):
    ...
    def __init__(self, pos_inds, neg_inds, masks, gt_masks, assign_result,
        ...
        if gt_box_masks != None:
            self.pos_box_mask = gt_box_masks[self.pos_assigned_gt_inds, :]
        else:
            self.pos_box_mask = torch.zeros_like(gt_masks)
...
class MaskPseudoSampler(BaseSampler):
    ...
    def sample(self, assign_result, masks, gt_masks, gt_box_masks = None, **kwargs):
        ...
        sampling_result = MaskSamplingResult(pos_inds, neg_inds, masks,
                                             gt_masks, assign_result, gt_flags, gt_box_masks)

...
#-------------------------------#
            ...
            for i in range(num_imgs):
                if stage < self.assign_stages:
                    ...
                    assign_results.append(assign_result)
                sampling_result = self.mask_sampler[stage].sample(
                    assign_results[i], scaled_mask_preds[i], gt_masks[i], gt_box_masks[i])
                sampling_results.append(sampling_result)
#----------------------------#            
            num_batch = len(gt_masks)
            #print("num_batch:", num_batch)
            ...
            '''
            single_stage_loss = self.mask_head[stage].loss_ls(
                single_stage_loss, levelset_feat, mask_targets[-2], mask_targets[-1]
            )
            '''

            single_stage_loss = self.mask_head[stage].loss_bi(
                single_stage_loss, scaled_mask_preds, img_color_similarity, mask_targets[-2], mask_targets[-1]
            )

    ...
    def loss_ls(self,
        ...
        for i in range(level_set_feat.shape[0]):
            ...
            if mask_pred.shape[0] == 0:
                ...
                loss_img_lst = self.loss_levelset(mask_scores_phi, img_target_wbox, pixel_num) * 0.25 # F_img
        ...
        loss['loss_levelset'] = loss_levelset
        return loss
...
#准备计算pairwise loss的color_similarity和box_mask，要求把第一维度的batch_size，扩充成伪标签实例的总和
        ...
        if flag_bt == 1:
            ...
            loss['loss_roi_pairwise'] = loss_bi
        else:
            loss['loss_roi_pairwise'] = mask_preds.sum() * 0
...
#拿出pos score和iou
        ...
        if flag == 0:
            return labels, label_weights, mask_targets, mask_weights,   pos_box_mask_list, pos_mask_list
        else: 
            return labels, label_weights, mask_targets, mask_weights

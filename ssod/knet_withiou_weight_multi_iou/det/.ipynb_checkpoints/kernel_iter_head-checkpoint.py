...
from .utils import find_max_ind
...
class KernelIterHead(BaseRoIHead):
    ...
    def __init__(self,
        ...
        self.thr = np.array([0.05+0.05*i for i in range(1, 18)])
...
#-------------------------#            
        ...
        if self.do_panoptic:
            ...
            for img_id in range(num_imgs):
                ...
                #print("iou_scores_per_img", iou_scores_per_img.shape) #torch.Size([100, 17])
...
#用来生成指定分类阈值的伪mask                
                iou_per_img = iou_scores_per_img[mask_indices]
                #print("iou_per_img.shape", iou_per_img.shape)
                """
                valid = scores_per_img > 0.3
                ...
                iou_per_img = iou_per_img[valid]
                
                single_result = self.mask_head[-1].get_seg_masks_iou(
                    masks_per_img, labels_per_img, scores_per_img, iou_per_img, 
...
#----------------------------------------#
        ...
        if self.do_panoptic:
            ...
            for img_id in range(num_imgs):
                ...
                #print("iou_per_img", iou_per_img.shape) #torch.Size([100, 17])
                #print("masks_per_img:", masks_per_img)
                #print(masks_per_img.shape)  #torch.Size([100, 200, 304])
                max_ind = find_max_ind(iou_per_img.cpu().detach().numpy())
                num_mask = iou_per_img.shape[0]
                
                thr = self.thr[max_ind].reshape(num_mask, 1)
                thr = torch.tensor(thr).to(iou_per_img.device)
                
                #print("thr", thr.shape) #torch.Size([100, 1])
                
                iou_per_img_numpy = iou_per_img.cpu().detach().numpy()
                iou_per_img_lst = []
                for i in range(num_mask):
                    iou_per_mask = iou_per_img_numpy[i, max_ind[i]]
                    iou_per_img_lst.append(iou_per_mask)
                #iou_per_img_numpy = np.array(iou_per_img_lst).reshape(num_mask, 1)
                iou_per_img_numpy = np.array(iou_per_img_lst)
                iou_per_img_tensor = torch.tensor(iou_per_img_numpy).to(iou_per_img.device)
                
                #print("iou_per_img_tensor", iou_per_img_tensor.shape) #torch.Size([100])
                #max_ind = torch.tensor(max_ind).to(iou_per_img.device)
                #iou_per_img = iou_per_img[:, max_ind]
                ...
                iou_list.append(iou_per_img_tensor)
                thr_list.append(thr)
        ...
        return results, label_list, score_list, iou_list, thr_list
...
#用来生成指定分类阈值的伪mask                
                ...
                iou_per_img_tensor, max_ind = torch.max(iou_per_img, 1)
                thr_tensor = torch.tensor(self.thr).to(iou_per_img.device)
                thr = thr_tensor[max_ind]
                ...
                single_result = self.mask_head[-1].get_seg_masks_multi_iou(
                    masks_per_img, labels_per_img, scores_per_img, thr, 
                    self.test_cfg, img_metas[img_id])
                results.append(single_result)

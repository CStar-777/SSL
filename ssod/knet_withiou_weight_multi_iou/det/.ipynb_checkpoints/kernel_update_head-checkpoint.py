...
from .utils import compute_mask_iou, compute_multi_mask_iou
...
#!增加一个分支，来预测iou的分数-----------------------#
        self.fc_objectness = nn.Linear(in_channels, 17)
...
#-------------------------------------------------#
        ...
        #print("iou_scores.shape", iou_scores.shape) #torch.Size([2, 100, 1]) #torch.Size([1, 100, 17])
...
#计算iou的二值交叉熵loss
                ...
                iou_labels = compute_multi_mask_iou(pos_mask_pred_1, pos_mask_targets_1)
                num_mask, thr = iou_labels.shape
                iou_labels = iou_labels.view(num_mask * thr, -1)
                ...
                if iou_scores is not None:
                    ...
                    pos_num_mask, pos_thr = pos_iou_scores.shape
                    pos_iou_scores = pos_iou_scores.view(pos_num_mask * pos_thr, -1)
                    #print("+"*100)
...
#可能无标签图片中没有大于0.5分类阈值的伪mask
    ...
    def get_seg_masks_iou(self, masks_per_img, labels_per_img, scores_per_img, iou_per_img,
                      test_cfg, img_meta):
        # resize mask predictions back
...
#!加入mask的大小
        #size = seg_masks.sum([1, 2]) #tensor (n)
        
        #print("size.shape", size.shape)
        
        
        bbox_result, segm_result, iou_result = self.segm2result_iou(seg_masks, labels_per_img,
                                                    scores_per_img, iou_per_img)
        return bbox_result, segm_result, iou_result
    
    def segm2result_iou(self, mask_preds, det_labels, cls_scores, iou_scores):
        num_classes = self.num_classes
        ...
        for idx in range(num_ins):
            ...
            iou_result[det_labels[idx]].append(iou_scores[idx])
            
        return bbox_result, segm_result, iou_result
    

    def get_seg_masks_multi_iou(self, masks_per_img, labels_per_img, scores_per_img, thr, 
                      test_cfg, img_meta):
        # resize mask predictions back
        
        seg_masks = self.rescale_masks(masks_per_img, img_meta)
        n, h, w = seg_masks.shape
        thr = thr.unsqueeze(-1).unsqueeze(-1).expand(n, h, w)
        seg_masks = seg_masks > thr      

...
#!-------------------------------------------------#            
    ...
    @staticmethod
    def transform_mask_scores(mask_scores, M, out_shape):
        if mask_scores.shape[0] == 0:
                return torch.zeros((0, out_shape[0], out_shape[1]))
        mask_tensor = mask_scores[:, None, ...]
        return (warp_affine(
                    mask_tensor,
                    M[None, ...].expand(mask_scores.shape[0], -1, -1),        #None在tensor中的作用类似torch.unsqueeze(),方便扩展维度，而不改变数据排列顺序。
                    #out_shape,
                    (out_shape[0], out_shape[1]),
                )
                .squeeze(1))
...
# filter_invalid_3(bbox=None, label=None, score=None, mask=None, iou=None, thr=0.0, iou_thr=0.7, min_size=None):
def filter_invalid_v0(bbox=None, label=None, score=None, mask=None, thr=0.0, min_size=0):
    if score is not None:
        valid = score > thr
        if bbox is not None:
            bbox = bbox[valid]
...
def filter_invalid_pa_baseline(thing_label=None, thing_score=None, thing_mask=None, stuff_label=None, stuff_score=None, stuff_mask=None, thr=0.0):
    if thing_score is not None:
        thing_valid = thing_score > thr
        if thing_label is not None:
            thing_label = thing_label[thing_valid]
        if thing_mask is not None:
            ...
            """
            #mask = mask[valid]
            thing_mask = BitmapMasks(thing_mask.masks[thing_valid.cpu().numpy()], thing_mask.height, thing_mask.width)
    if stuff_score is not None:
        stuff_valid = stuff_score > thr
        if stuff_label is not None:
            stuff_label = stuff_label[stuff_valid]
        if stuff_mask is not None:
            ...
            """
            #mask = mask[valid]
            stuff_mask = BitmapMasks(stuff_mask.masks[stuff_valid.cpu().numpy()], stuff_mask.height, stuff_mask.width)
    return thing_mask, thing_label, thing_score, stuff_mask, stuff_label, stuff_score


def filter_invalid_pa_da(thing_label=None, thing_score=None, thing_mask=None, thing_iou=None, stuff_label=None, stuff_score=None, stuff_mask=None, stuff_iou=None, thr=0.0, iou_thr=0.0):
    if thing_score is not None:
        thing_valid0 = thing_score > thr
        thing_valid1 = thing_iou > iou_thr
        thing_valid = thing_valid0 * thing_valid1
        if thing_score is not None:
            thing_score = thing_score[thing_valid]
        if thing_label is not None:
            thing_label = thing_label[thing_valid]
        if thing_mask is not None:
            thing_mask = BitmapMasks(thing_mask.masks[thing_valid.cpu().numpy()], thing_mask.height, thing_mask.width)
        if thing_iou is not None:
            thing_iou = thing_iou[thing_valid]
            
    if stuff_score is not None:
        stuff_valid0 = stuff_score > thr
        stuff_valid1 = stuff_iou > iou_thr
        stuff_valid = stuff_valid0 * stuff_valid1
        if stuff_score is not None:
            stuff_score = stuff_score[stuff_valid]
        if stuff_label is not None:
            stuff_label = stuff_label[stuff_valid]
        if stuff_mask is not None:
            stuff_mask = BitmapMasks(stuff_mask.masks[stuff_valid.cpu().numpy()], stuff_mask.height, stuff_mask.width)
        if stuff_iou is not None:
            stuff_iou = stuff_iou[stuff_valid]
    return thing_mask, thing_label, thing_score, thing_iou, stuff_mask, stuff_label, stuff_score, stuff_iou

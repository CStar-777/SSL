...
class SslKnet_weight_class_cp(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_weight_class_cp, self).__init__(
        ...
        if train_cfg is not None:
            ...
            # self.ulb_dset_len = 106459
            # self.register_buffer("ulb_dset_len", 100000)
            self.register_buffer(
                "scores", torch.zeros((self.num_classes, self.num_scores))
            )
            self.register_buffer("class_acc", torch.zeros((self.num_classes,)))
            self.register_buffer(
                "sel_cnt",
                torch.zeros(
                    (self.num_classes,),
    ...
    def compute_pseudo_label_loss(self, img, img_metas, teacher_info):
        # # 进行copy paste操作 cx add
        # img, teacher_info = self.teacher.roi_head.copyPaste_test(img, img_metas, teacher_info)
        ...
        if teacher_info["det_scores"][0].shape != torch.Size([0]):
            if teacher_info["det_scores"][0][0].cpu() >= 0.5:
                (
                    ...
                    teacher_info,
                    pseudo_masks,
                ) = self.teacher.roi_head.copyPaste_test_2(
                    img, img_metas, teacher_info, pseudo_masks
                )

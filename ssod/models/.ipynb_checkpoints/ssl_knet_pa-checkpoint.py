...
from .utils import Transform2D, filter_invalid_2, filter_invalid_1, filter_invalid_pa_baseline
...
class SslKnet_Pa(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_Pa, self).__init__(
...
#------------------------------------------#        
        ...
        rpn_results = self.student.rpn_head.forward_train(x, img_metas, gt_masks,
                                                  gt_labels, gt_sem_seg = gt_stuff_masks, gt_sem_cls = gt_stuff_labels)
...
#------------------------------------#
        ...
        losses = self.student.roi_head.forward_train(
            ...
            gt_masks,
            gt_labels,
            gt_sem_seg = gt_stuff_masks, gt_sem_cls = gt_stuff_labels)    
...
#--------------------------------------------#
        score_list, label_list, mask_list, stuff_score_list, stuff_label_list, stuff_mask_list = self.teacher.roi_head.teacher_test(x_feats, proposal_feats, mask_preds, cls_scores, img_metas)
...
#-两种过滤方式。带iou的和不带iou的
#----------------------------------#
        det_masks, det_labels, det_scores, det_stuff_masks, det_stuff_labels, det_stuff_scores = list(   
            zip(
                *[
                    filter_invalid_pa_baseline(
                        thing_mask=mask,
                        thing_label=label,
                        thing_score=score,
                    ...
                    for score, label, mask, stuff_score, stuff_label, stuff_mask in zip(
                        score_list, label_list, mask_list, stuff_score_list, stuff_label_list, stuff_mask_list
                    )
                ]
            )
        )
        ...
        teacher_info["det_stuff_masks"] = det_masks
        teacher_info["det_stuff_labels"] = det_labels

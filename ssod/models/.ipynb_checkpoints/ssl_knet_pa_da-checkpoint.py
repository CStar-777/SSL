...
from ssod.utils import log_image_with_boxes, log_every_n, isVisualbyCount, log_image_with_masks_without_box

from .multi_stream_detector import MultiSteamDetector
from .utils import Transform2D, filter_invalid_2, filter_invalid_1, filter_invalid_pa_baseline, filter_invalid_pa_da
...
class SslKnet_Pa_Da(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SslKnet_Pa_Da, self).__init__(
...
#！由于两个通道的数据增强不一致，需要将teacher的mask变换到student上，mask的变换----------------------#
        ...
        #torch.Size([8]), tensor([0, 0, 0, 0, 0, 0, 0, 0], device='cuda:0')
        gt_labels = teacher_info["det_labels"]
        
        pseudo_stuff_masks = self._transform_mask(
            teacher_info["det_stuff_masks"],   #list(bitmapmasks)    3, 
            M,
            [meta["img_shape"] for meta in img_metas],
        )
        #torch.Size([8]), tensor([0, 0, 0, 0, 0, 0, 0, 0], device='cuda:0')
        gt_stuff_labels = teacher_info["det_stuff_labels"] 
        ...
        if flag == 1:
            ...
            stuff_mask_ori = self._transform_mask(
                teacher_info["det_stuff_masks"],   #list(bitmapmasks)    3, 
                M1,
                [meta["ori_shape"] for meta in img_metas],
            )
            ...
            for i in range(len(img_metas)):
                ...
                stuff_mask_vis = stuff_mask_ori[i].to_tensor(torch.float, img[0].device).cpu().detach()
                mask_vis = torch.cat((mask_vis, stuff_mask_vis), dim=0)
                mask_vis = mask_vis > 0.5
                
                label_vis = gt_labels[i].cpu().detach()
                stuff_label_vis = gt_stuff_labels[i].cpu().detach()
                label_vis = torch.cat((label_vis, stuff_label_vis), dim=0)
                
                #mask_vis torch.Size([16, 467, 640])
...
#------------------------------------------#        
        ...
        stuff_gt_masks_tensor = []
        for i, stuff_gt_mask in enumerate(pseudo_stuff_masks):
            mask_tensor = stuff_gt_mask.to_tensor(torch.float, gt_labels[0].device)
            if stuff_gt_mask.width != pad_W or stuff_gt_mask.height != pad_H:
                pad_wh = (0, pad_W - stuff_gt_mask.width, 0, pad_H - stuff_gt_mask.height)
                mask_tensor = F.pad(mask_tensor, pad_wh, value=0)
            ...
            if flag == 1:            
                ...
                mask_vis = mask_tensor.cpu().detach()
                mask_vis = mask_vis > 0.5
                label_vis = gt_stuff_labels[i].cpu().detach()
            ...
            if mask_tensor.shape[0] == 0:
                ...
                stuff_gt_masks_tensor.append(
                    F.interpolate(
                        ...
                        mode='bilinear',
                        align_corners=False)[0])
        gt_stuff_masks = stuff_gt_masks_tensor
        gt_ious = teacher_info["det_ious"] 
        gt_stuff_ious = teacher_info["det_stuff_ious"] 
        gt_scores =  teacher_info["det_scores"]
        gt_stuff_scores =  teacher_info["det_stuff_scores"]
        
        rpn_results = self.student.rpn_head.forward_train(x, img_metas, gt_masks,
                                                  gt_labels,  gt_scores, gt_ious, gt_stuff_masks, gt_stuff_labels, gt_stuff_scores, gt_stuff_ious)
...
#------------------------------------#
        ...
        losses = self.student.roi_head.forward_train(
            ...
            gt_scores,
            gt_ious,
            gt_sem_seg = gt_stuff_masks, gt_sem_cls = gt_stuff_labels, gt_sem_score=gt_stuff_scores, gt_sem_iou=gt_stuff_ious)
...
#-------------------------------------------------#
    ...
    def extract_teacher_info(self, img, img_metas, proposals=None, **kwargs):
        ...
        self.teacher.rpn_head.training = False
        #start2 = time.time()
...
#--------------------------------------------#
        score_list, iou_list, label_list, mask_list, stuff_score_list, stuff_iou_list, stuff_label_list, stuff_mask_list = self.teacher.roi_head.teacher_test(x_feats, proposal_feats, mask_preds, cls_scores, img_metas)
...
#-两种过滤方式。带iou的和不带iou的
#----------------------------------#
        det_masks, det_labels, det_scores, det_ious, det_stuff_masks, det_stuff_labels, det_stuff_scores, det_stuff_ious = list(   
            zip(
                *[
                    filter_invalid_pa_da(
                        thing_mask=mask,
                        thing_label=label,
                        thing_score=score,
                        thing_iou=iou,
                        stuff_mask=stuff_mask,
                        stuff_label=stuff_label,
                        stuff_score=stuff_score,
                        stuff_iou=stuff_iou,
                        thr=thr,
                        iou_thr=iou_thr,
                    )
                    for score, label, mask, iou, stuff_score, stuff_label, stuff_mask, stuff_iou in zip(
                        score_list, label_list, mask_list, iou_list, stuff_score_list, stuff_label_list, stuff_mask_list, stuff_iou_list
                    )
                ]
            )
        )
        ...
        teacher_info["det_stuff_masks"] = det_stuff_masks
        teacher_info["det_stuff_labels"] = det_stuff_labels
        teacher_info["det_stuff_ious"] = det_stuff_ious
        teacher_info["det_stuff_scores"] = det_stuff_scores

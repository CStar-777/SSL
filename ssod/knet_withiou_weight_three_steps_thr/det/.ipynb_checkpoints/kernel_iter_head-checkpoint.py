...
class KernelIterHead(BaseRoIHead):
    ...
    def forward_train(self,
        ...
        if self.mask_head[0].mask_upsample_stride > 1:
            ...
            prev_mask_preds = mask_preds.detach() # m0
...
#----------------------------------------#
        ...
        if self.do_panoptic:
            ...
            for img_id in range(num_imgs):
                ...
                masks_per_img = scaled_mask_preds[img_id][mask_indices]        
                #mask_pred.append(masks_per_img.unsqueeze(0).sigmoid().squeeze(0))
                iou_per_img = iou_scores_per_img.flatten(0)[mask_indices]
        ...
        return results, mask_pred, label_list, score_list, iou_list
    ...
    def teacher_test_scale(self,
        ...
        for stage in range(self.num_stages):
            ...
            iou_scores = mask_results['iou_scores']
        num_classes = self.mask_head[-1].num_classes
        scaled_mask_preds = self.test_rescale_mask(scaled_mask_preds, scale, mode, img_metas)
        results = []
        mask_pred = []
        score_list = []
        label_list = []
...
#----------------------------------------#
    ...
    def rescale_masks_scale(self, masks_per_img, img_meta):
        h, w, _ = img_meta['img_shape']
        masks_per_img = F.interpolate(
            masks_per_img.unsqueeze(0).sigmoid(),
            size=img_meta['batch_input_shape'],
        ...
        masks_per_img = masks_per_img[:, :, :h//2, :w//2]
        masks_per_img = F.interpolate(
            masks_per_img,
            size=(h, w),
            mode='bilinear',
    ...
    def simple_test_iou(self,
        ...
        # scale部分
        scaled_mask_preds = self.test_rescale_mask(scaled_mask_preds, scale)
...
#用来生成指定分类阈值的伪mask                
                ...
                '''
                valid = scores_per_img > 0.15
                #valid_1 = iou_per_img > 0.75
                #valid = valid_0 * valid_1
    ...
    def test_rescale_mask(self, masks_pred, scale, mode, img_metas):
        if scale == 1:
            return masks_pred
        _, _, h, w = masks_pred.shape
        if mode == "down_wpad":
            masks_pred = masks_pred[:, :, :h//scale, :w//scale]
            ...
            h, w = img_metas[0]["downsample_shape"]

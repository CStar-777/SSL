...
#------------------------------------#        
        ...
        losses = self.loss(scaled_mask_preds, cls_scores, scaled_seg_preds,
                           proposal_feats, flag, is_scale, *mask_targets)
    ...
    def loss(self,
             ...
             **kwargs):
        if is_scale == None:
            str_loss_rpn_cls = "loss_rpn_cls"
            str_rpn_pos_acc = "rpn_pos_acc"
            str_loss_rpn_mask = "loss_rpn_mask"
            str_loss_rpn_dice = "loss_rpn_dice"
            str_loss_rpn_seg = "loss_rpn_seg"
        else:
            str_loss_rpn_cls = "loss_rpn_cls" + is_scale
            str_rpn_pos_acc = "rpn_pos_acc" + is_scale
            str_loss_rpn_mask = "loss_rpn_mask" + is_scale
            str_loss_rpn_dice = "loss_rpn_dice" + is_scale
            str_loss_rpn_seg = "loss_rpn_seg" + is_scale
        losses = dict()
...
#------------------------------------------#        
        ...
        if cls_scores is not None:
            ...
            losses[str_loss_rpn_cls] = self.loss_cls(
                cls_scores.view(num_preds, -1),
                labels,
                label_weights,
                avg_factor=avg_factor,
                reduction_override=reduction_override)
            losses[str_rpn_pos_acc] = accuracy(
...
#-------------------------------------------------#            
            ...
            '''
            #rint("pos_mask_weight_1",pos_mask_weight_1, pos_mask_weight_1.shape)
            #rint("H, W", H, W)
            #print("pos_mask_targets", mt, pos_mask_targets.shape)
            
            
            
            losses[str_loss_rpn_mask] = loss_mask(pos_mask_pred,
                                                     pos_mask_targets,
                                               pos_mask_weight_1)
            losses[str_loss_rpn_dice] = self.loss_dice(pos_mask_pred,
                                                     pos_mask_targets,
                                                    pos_mask_weight)
#---------------------------------#
            #print("-"*100)
            ...
            losses[str_loss_rpn_mask] = mask_pred.sum() * 0
            losses[str_loss_rpn_dice] = mask_pred.sum() * 0
            if self.loss_rank is not None:
...
#---------------------------------------------------------------------#
                if pos_inds.any():
                    losses[str_loss_rpn_seg] = self.loss_seg(
                        flatten_seg,
                        flatten_seg_target,
                        avg_factor=num_dense_pos)
                else:
                    losses[str_loss_rpn_seg] = seg_preds.sum() * 0
            else:
                cls_channel = seg_preds.shape[1]
                ...
                losses[str_loss_rpn_seg] = self.loss_seg(flatten_seg,
                                                       flatten_seg_target)
...
#修改分类的权重，>=0.5之后就不再使用动态阈值                
                ...
                label_weights[pos_inds] = pos_score * pos_score * 4.0
                #print("检查是否需要梯度"+"="*50)
            ...
            if flag == 1:
                ...
                #mask_weights[pos_inds] = 1
                mask_weights[pos_inds] = pos_iou * pos_iou * 2.5
                '''
                import numpy as np
